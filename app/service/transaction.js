...
class TransactionService extends Service {
  async getTransaction(id) {
    ...
    const {Address: RawAddress} = this.app.CcSinfo.lib
  ...
  async getRawTransaction(id) {
    ...
    const {Transaction: RawTransaction, Input, Output, OutputScript} = this.app.CcSinfo.lib
  ...
  async sendRawTransaction(data) {
    let client = new this.app.CcSinfo.rpc(this.app.config.CcSinfo.rpc)
  ...
  transformInput(input, index, transaction, {brief}) {
    const {InputScript, OutputScript} = this.app.CcSinfo.lib
  ...
  transformOutput(output, index, {brief}) {
    const {OutputScript} = this.app.CcSinfo.lib
  ...
  async transformQRC20Transfers(outputs) {
    const TransferABI = this.app.CcSinfo.lib.Solidity.qrc20ABIs.find(abi => abi.name === 'Transfer')
    let result = []
  ...
  async transformQRC20UnconfirmedTransfers(outputs) {
    const {OutputScript, Solidity} = this.app.CcSinfo.lib
  ...
  async getContractTransaction(receiptId) {
    const {Address: RawAddress, OutputScript} = this.app.CcSinfo.lib
    const {
